...
function showTab(name) {
  ...
  if (bar) bar.style.display = name === 'calc' ? 'block' : 'none';
...
async function refreshCalcPrices() {
  ...
  s('cr-fx',  S.fx.toLocaleString());
  s('cr-btc', Math.round(calcBtcPrice).toLocaleString());
  s('sub-btc-price', Math.round(calcBtcPrice).toLocaleString());
  s('sub-krw-rate',  S.fx.toLocaleString());
  const inp = document.getElementById('calc-input');
  if (inp && inp.value) calcConvert(inp.value, calcUnit);
}

function calcConvert(val, unit) {
  const n = parseFloat(val) || 0;
  const premium = parseFloat(document.getElementById('calc-premium')?.value || 0) / 100;
  const s = (id, txt) => { const el = document.getElementById(id); if(el) el.textContent = txt; };
  let usdt = 0;
  if (n > 0) {
    if (unit === 'USD' || unit === 'USDT') usdt = n * (1 + premium);
    else if (unit === 'KRW')  usdt = (n / S.fx) * (1 + premium);
    else if (unit === 'BTC')  usdt = n * calcBtcPrice * (1 + premium);
    else if (unit === 'Sats') usdt = (n / 100000000) * calcBtcPrice * (1 + premium);
  }
  const btcV = calcBtcPrice > 0 ? usdt / calcBtcPrice : 0;
  s('val-usdt', n > 0 ? usdt.toFixed(2) : '-');
  s('val-usd',  n > 0 ? '$' + usdt.toFixed(2) : '-');
  s('val-krw',  n > 0 ? '₩' + Math.round(usdt * S.fx).toLocaleString() : '-');
  s('val-btc',  n > 0 && btcV > 0 ? btcV.toFixed(8) : '-');
  s('val-sats', n > 0 && btcV > 0 ? Math.round(btcV * 100000000).toLocaleString() : '-');
  s('sub-btc-price', Math.round(calcBtcPrice).toLocaleString());
  s('sub-krw-rate',  S.fx.toLocaleString());
  s('cr-fx',  S.fx.toLocaleString());
  s('cr-btc', Math.round(calcBtcPrice).toLocaleString());
...
function setUnit(unit) {
  ...
  ['BTC','KRW','USD','Sats','USDT'].forEach(u => {
    const opt = document.getElementById('uopt-'+u);
    const chk = document.getElementById('unit-check-'+u);
    if (opt) opt.style.color = u === unit ? 'var(--green)' : 'var(--text2)';
    if (chk) chk.style.display = u === unit ? 'inline' : 'none';
  });
  const inp = document.getElementById('calc-input');
  if (inp && inp.value) calcConvert(inp.value, unit);
}
...
function clearCalc() {
  const inp = document.getElementById('calc-input');
  if (inp) inp.value = '';
  ['val-usdt','val-usd','val-krw','val-btc','val-sats'].forEach(id => {
...
function copyCalcVal(id) {
  const el = document.getElementById(id);
  if (!el || el.textContent === '-') return;
  navigator.clipboard.writeText(el.textContent.replace(/[₩$,]/g,'').trim()).then(() => toast('복사됐어요'));
}
